...
first_bogota_date_col = 'FORMAT_DATE("%Y-%m-%d", DATE_SUB(SAFE_CAST(fechaconsu AS DATE), INTERVAL 3 DAY))'
second_bogota_date_col = 'FORMAT_DATE("%Y-%m-%d", DATE_SUB(SAFE_CAST(fechadiagn AS DATE), INTERVAL 5 DAY))'

# Max Symptoms
bogota_sql_max_date = f"""
                    SELECT MAX(DATE(IFNULL({first_bogota_date_col}, {second_bogota_date_col}))) as max_date
                    FROM `servinf-unacast-prod.AlcaldiaBogota.positivos_agg_fecha`
                    WHERE SAFE_CAST(IFNULL({first_bogota_date_col}, {second_bogota_date_col}) AS DATE) IS NOT NULL
...
# Min Symptoms
bogota_sql_min_date = f"""
                    SELECT MIN(DATE(IFNULL({first_bogota_date_col}, {second_bogota_date_col}))) as min_date
                    FROM `servinf-unacast-prod.AlcaldiaBogota.positivos_agg_fecha`
                    WHERE SAFE_CAST(IFNULL({first_bogota_date_col}, {second_bogota_date_col}) AS DATE) IS NOT NULL
                """
...
def get_positive_max_dates(client):
    ...
    max_dates = {}
    max_dates['bogota'] = pd.to_datetime(utils.run_simple_query(
        client, bogota_sql_max_date).max_date.iloc[0])

    for city in cities_with_positives:
        if city != 'bogota':
            max_dates[city] = pd.to_datetime(utils.run_simple_query(
                client, generic_sql_max_date.format(table_name=city)).max_date.iloc[0])
...
def get_positive_min_dates(client):
    ...
    min_dates = {}
    min_dates['bogota'] = pd.to_datetime(utils.run_simple_query(
        client, bogota_sql_min_date).min_date.iloc[0])

    for city in cities_with_positives:
        if city != 'bogota':
            min_dates[city] = pd.to_datetime(utils.run_simple_query(
                client, generic_sql_min_date.format(table_name=city)).min_date.iloc[0])
...
def location_id_supported_on_date(client, location_id, current_date, df_codes, max_dates, min_dates):
    ...
    up_to_date = positives_inside_date(
        client, location_id, current_date, df_codes, max_dates, min_dates)
