...
def main():
    ...
    df_codes = utils.get_distance_to_housing_code_coverage(client)
    
    # Only stays with colombia
    df_codes = df_codes[df_codes.code_depto.apply(lambda c: c.startswith('CO.'))]
    
    print('Updates Distance to Housing')
    ...
    for ind, row in selected.iterrows():
        ...
        utils.update_distance_to_housing(client, code_depto, monday_str, sunday_str)
